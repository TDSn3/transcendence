...
function Channel(param: { id:number, name: string }) {
...
function Channels() {
  ...
  const handleSubmit: any = (e: any) => {
    e.preventDefault();
    axios.post('http://localhost:5001/api/channels', {
      intraId: (decodeToken(document.cookie).sub), name: channelName, password: channelPassword, private: channelPrivate,
    })
      .then(() => {
  ...
  return (
    <div>
      ...
      <div className="channels">
        {
          channelsNames.map((value: any) => <Channel key={value.id} id={value.id} name={value.name} />)
        }
      </div>
      <Popup className="create" trigger={buttonPopup} setTrigger={setButtonPopup}>
        ...
        <form className="create-form" onSubmit={handleSubmit}>
          ...
          <div className="create-form-end">
            Private:
            {' '}
            <input type="checkbox" onChange={() => { setChannelPrivate(!channelPrivate); }} />
